...
<body>
  ...
  <div class="card chart-card">
    <div class="chart-range" aria-label="Chart Zeitraum">
      <button type="button" data-chart-points="240">1h</button>
      <button type="button" data-chart-points="1440">6h</button>
      <button type="button" data-chart-points="2880">12h</button>
      <button type="button" data-chart-points="5760" class="active" aria-pressed="true">24h</button>
    </div>
    <div class="chart">
...
<script>
  ...
  let websocket = null;
  let flowChart = null;
  const maxChartPoints = 5760;
  const chartLabels = new Array(maxChartPoints);
  const chartValues = new Array(maxChartPoints);
  let chartWriteIndex = 0;
  let chartPointCount = 0;
  let selectedChartPoints = maxChartPoints;
  ...
  function resetChartBuffer() {
    chartLabels.fill(undefined);
    chartValues.fill(undefined);
    chartWriteIndex = 0;
    chartPointCount = 0;
  }
  ...
  function updateChartFromBuffer() {
    ...
    const orderedLabels = [];
    const orderedValues = [];
    const visiblePointCount = Math.min(chartPointCount, selectedChartPoints);
    const oldestIndex = chartPointCount === maxChartPoints ? chartWriteIndex : 0;
    const startIndex = (oldestIndex + chartPointCount - visiblePointCount) % maxChartPoints;

    for (let i = 0; i < visiblePointCount; i++) {
      const bufferIndex = (startIndex + i) % maxChartPoints;
      orderedLabels.push(chartLabels[bufferIndex]);
      orderedValues.push(chartValues[bufferIndex]);
    }

    flowChart.data.labels = orderedLabels;
    flowChart.data.datasets[0].data = orderedValues;
    flowChart.update();
  }

  function addChartPoint(label, value, shouldUpdate) {
    chartLabels[chartWriteIndex] = label;
    chartValues[chartWriteIndex] = value;
    chartWriteIndex = (chartWriteIndex + 1) % maxChartPoints;
    chartPointCount = Math.min(chartPointCount + 1, maxChartPoints);
  ...
  function initChartRangeControls() {
    const buttons = document.querySelectorAll(".chart-range button");

    buttons.forEach(function (button) {
      button.addEventListener("click", function () {
        selectedChartPoints = parseInt(button.dataset.chartPoints, 10);

        buttons.forEach(function (rangeButton) {
  ...
  function initWebSocket() {
    ...
    websocket.onmessage = function (event) {
      ...
      if (event.data === "chart_begin") {
        resetChartBuffer();
        return;
      }

      if (event.data === "chart_end") {
        updateChartFromBuffer();
        return;
      }

      if (event.data.startsWith("chart_batch|")) {
        const batch = event.data.substring("chart_batch|".length);
        if (batch.length > 0) {
          const points = batch.split(";");
          for (let i = 0; i < points.length; i++) {
            const point = points[i].split(",");
            if (point.length >= 2) {
              addChartPoint(point[0], parseInt(point[1], 10), false);
            }
          }
        }
        return;
      ...
      if (event.data.startsWith("chart|")) {
        const chartParts = event.data.split("|");
        if (chartParts.length >= 3) {
          addChartPoint(chartParts[1], parseInt(chartParts[2], 10), true);
        }
        return;
      }
